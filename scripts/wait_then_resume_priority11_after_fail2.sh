#!/usr/bin/env bash
# After the fail-2 resume dispatcher exits, continue the remaining priority-11 books.
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="${PWD}"
WATCH="${ROOT}/book_analysis/semantica/queue/wait-priority-11-after-fail2.log"
OUT="${ROOT}/book_analysis/semantica/queue/dispatcher-oauth-apply.out"
FAIL_PID="${1:-3740704}"

log() { echo "$(date -Iseconds) $*" | tee -a "${WATCH}"; }

log "wait for fail-2 dispatcher pid=${FAIL_PID}"
while kill -0 "${FAIL_PID}" 2>/dev/null; do
  sleep 20
done
log "fail-2 dispatcher gone"
